...
HOST_IPV6 = "::"  # IPv6通配地址，监听本机所有IPv6网卡（对应IPv4的0.0.0.0）
PORT = 40000      # 和客户端端口保持一致
...
# 单个客户端连接处理（逻辑无修改，自动兼容IPv6地址元组）
def handle_client(conn, addr, current_pwd_hash):
    # addr 格式 (ipv6地址, 端口, flowinfo, scopeid)
    client_ip = addr[0]
    print(f"[新IPv6连接] [{client_ip}]:{addr[1]} 已接入")
    auth_ok = False
    ...
    try:
        ...
        if client_hash_data == current_pwd_hash:
            ...
            auth_ok = True
            print(f"[[{client_ip}]] 密码校验通过，允许操作")
        else:
            err_msg = "AUTH_FAILED: 密码错误，连接关闭"
            send_msg(conn, err_msg)
            conn.close()
            print(f"[[{client_ip}]] 密码错误，断开连接")
            return
        ...
        while True:
            ...
            if not cmd_data:
                break
            print(f"[[{client_ip}]] 收到指令: {cmd_data}")
            ...
            # 删除文件
                ...
                try:
                    ...
                    send_msg(conn, msg)
                    print(f"[[{client_ip}]] 删除文件成功：{safe_name}")
        ...
        print(f"[{client_ip}] IPv6连接异常: {e}")
    finally:
        conn.close()
        print(f"[[{client_ip}]] IPv6连接已断开")
...
def start_server():
    ...
    # IPv6 bind元组 (地址, 端口)
    server_sock.bind((HOST_IPV6, PORT))
    server_sock.listen(128)
    print("="*60)
    print(f"IPv6云存储服务端启动成功，监听所有本机IPv6地址 ::{PORT}")
    print(f"当前访问密码：{current_pwd}")
    print(f"磁盘剩余预警阈值：{format_size(WARN_FREE_SPACE)}")
    print("输入 setpwd 修改访问密码，输入 start 启动IPv6监听")
    print("="*60)
